...
const getStartOfDay = (
    date: ZonedDateTime | CalendarDate,
    timeZone: string
): ZonedDateTime => {
...
const getDaysInVisibleMonthGrid = (
...
const getEventsForDay = (
    ...
    return allEvents
        .filter((event) => {
            ...
            return (
                event.start.compare(dayEnd) < 0 &&
                event.end.compare(dayStart) > 0
            );
        })
...
const PositionedEvent = ({
    ...
    const formatTime = (dateTime: ZonedDateTime) =>
        timeFormatter.format(dateTime.toDate());
    ...
    const clampedStart =
        startZoned.compare(dayStart) < 0 ? dayStart : startZoned;
...
const MonthView = ({
    ...
    const formatTimeForMonth = (dateTime: ZonedDateTime) =>
        timeFormatter.format(dateTime.toDate());
    ...
    const weekdays = useMemo(() => {
        ...
        return Array.from({ length: 7 }).map((_, i) =>
            shortWeekdayFormatter.format(
                firstDay.add({ days: i }).toDate(timeZone)
            )
        );
    ...
    const visibleEvents = zonedEvents.filter((event) => {
        ...
        return (
            eventStartDay.compare(weekEnd) <= 0 &&
            eventEndDay.compare(weekStart) >= 0
        );
    });
    ...
    const targetDateForFooter =
        selectedDate ||
        (isSameMonth(currentMonthDate, todayCalDate) ? todayCalDate : null);
        ...
        eventsForFooter = zonedEvents
            .filter((event) => {
                ...
                return (
                    ...
                    (event.start.compare(targetZonedEnd) < 0 &&
                        event.end.compare(targetZonedStart) > 0)
                );
            })
            .sort((a, b) => a.start.compare(b.start));
    }
    ...
    const calculateEventSpan = useCallback(
        (event: ZonedEvent, currentDate: CalendarDate) => {
            ...
            while (
                checkDayIndex <= weekEndIndex &&
                checkDay.compare(eventEndDay) <= 0
            ) {
                spanDays++;
    ...
    const eventRenderMap = useMemo(() => {
        ...
        days.forEach((day, dayIndex) => {
            ...
            eventsOnThisDay.forEach((event) => {
                ...
                const eventStartDay = toCalendarDate(event.start);
                const isNewWeekSegment = isWeekBoundaryStart(
                    dayIndex,
                    eventStartDay,
                    day
                );
                ...
                renderMap
                    .get(dateKey)!
                    .push({ event, span, isPlaceholder: false, row });
                ...
                for (let offset = 1; offset < span; offset++) {
                    ...
                    renderMap
                        .get(nextDayKey)!
                        .push({ event, span: 0, isPlaceholder: true, row });
                }
            });
        });
        ...
        <div className={cx("flex flex-1 flex-col", className)}>
            ...
            >
                {days.map((date, index) => {
                    ...
                    const isCurrentMonthFlag = isSameMonth(
                        date,
                        currentMonthDate
                    );
                    ...
                    const eventsToShow = eventsToRender.slice(
                        0,
                        MAX_EVENTS_PER_CELL
                    );
                    ...
                    return (
                        <CalendarMonthViewCell
                            ...
                            onClick={() =>
                                isCurrentMonthFlag && setSelectedDate(date)
                            }
                        >
                            <div className="flex gap-1 max-md:pl-1 md:flex-col">
                                {eventsToShow.map(
                                    ({ event, span, isPlaceholder }) => {
                                        ...
                                        const eventStartDay = toCalendarDate(
                                            event.start
                                        );
                                        const eventEndDay = toCalendarDate(
                                            event.end
                                        );
                                        const startsToday = isSameDay(
                                            date,
                                            eventStartDay
                                        );
                                        const endsToday = isSameDay(
                                            date,
                                            eventEndDay
                                        );
                                        const continuesPrior =
                                            eventStartDay.compare(date) < 0;
                                        const continuesAfter =
                                            eventEndDay.compare(date) > 0;
                                        ...
                                        const eventEndsAtDayEnd = isSameDay(
                                            eventEndDay,
                                            toCalendarDate(
                                                getEndOfDay(event.end, timeZone)
                                            )
                                        );
                ...
                <div className="border-t border-secondary px-4 py-5 md:hidden">
                    <h3 className="text-sm font-semibold text-primary">
                        {fullDateFormatter.format(
                            targetDateForFooter.toDate(timeZone)
                        )}
                    </h3>
                    ...
                    {eventsForFooter.length > 0 && (
                        <div className="mt-4 flex flex-col gap-1.5">
                            {eventsForFooter.slice(0, 3).map((event) => {
                                ...
                                const eventStartDay = getStartOfDay(
                                    event.start,
                                    timeZone
                                );
                                const eventEndDay = getEndOfDay(
                                    event.end,
                                    timeZone
                                );
                                const isAllDay =
                                    event.start.compare(eventStartDay) <= 0 &&
                                    event.end.compare(eventEndDay) >= 0;

                                const supportingText = isAllDay
                                    ? "All day"
                                    : formatTimeForMonth(event.start);
                    ...
                    {eventsForFooter.length === 0 && (
                        <p className="mt-4 text-xs font-semibold text-quaternary">
...
const MobileSingleDayGrid = ({
    ...
    const dayStart = useMemo(
        () => getStartOfDay(dayToDisplay, timeZone),
        [dayToDisplay, timeZone]
    ); // Calculate once
        ...
        <div className={cx("relative flex-1", className)}>
            {Array.from({ length: 48 }).map((_, slotIndex) => (
                <CalendarDwViewCell
                    ...
                    className={cx(
                        "before:border-r-0",
                        slotIndex === 47 && "before:border-b-0"
                    )}
                />
            ))}
            ...
            {dayEvents.map((event, index) => {
                const overlapIndex = dayEvents.filter(
                    (e, i) =>
                        i < index &&
                        event.start.compare(e.end) < 0 &&
                        event.end.compare(e.start) > 0
                ).length;
    ...
    return (
        <div className="flex flex-col border-secondary">
            <div
                ...
                style={{ minHeight: `${48 * slotHeight}px` }}
            >
                {Array.from({ length: 48 }).map((_, slotIndex) => (
                    <CalendarDwViewCell
...
const WeekView = ({
    ...
    const showTimeMarker =
        ...
        localCurrentTime.compare(
            toZoned(currentWeekEnd, timeZone).add({ days: 1 })
        ) < 0;
    ...
    const visibleEvents = zonedEvents.filter((event) => {
        ...
        const eventEndDay = toCalendarDate(event.end);
        return (
            eventStartDay.compare(currentWeekEnd) <= 0 &&
            eventEndDay.compare(currentWeekStart) >= 0
        );
    });
    ...
    const earliestEventTimeInWeek = useMemo(() => {
        ...
        return visibleEvents.reduce((earliest, current) => {
            return current.start.compare(earliest.start) < 0
                ? current
                : earliest;
        }).start;
    ...
    useLayoutEffect(() => {
        ...
        if (
            ...
            earliestEventTimeInWeek
        ) {
            const startMinutes =
                earliestEventTimeInWeek.hour * 60 +
                earliestEventTimeInWeek.minute;
            // Calculate target scroll, ensuring it's not negative
            let targetScrollTop = Math.max(
                0,
                (startMinutes / 30) * SLOT_HEIGHT
            );
            ...
            <div className="sticky top-0 z-10 grid grid-cols-7 bg-primary pl-18 shadow-sm dark:border-b dark:border-secondary">
                {days.map((day) => {
                    ...
                    const dayToHighlight =
                        selectedDate &&
                        day.compare(currentWeekStart) >= 0 &&
                        day.compare(currentWeekEnd) <= 0
                            ? selectedDate
                            : currentWeekStart;
                    const isSelectedFlag = isSameDay(day, dayToHighlight);
                    const weekDayShort = shortWeekdayFormatter.format(
                        day.toDate(timeZone)
                    );
            ...
            >
                ...
                <div className="flex h-max w-18 flex-col border-r border-secondary">
                    {hours.map((hour) => {
                        ...
                        const timeString = hourOnlyFormatter.format(
                            toZoned(time, timeZone).toDate()
                        );
                        return (
                            <CalendarRowLabel key={`time-${hour}`}>
                                {timeString}
                            </CalendarRowLabel>
                        );
                    })}
...
const DayView = ({
    ...
    const showTimeMarker = isSameDay(
        toCalendarDate(localCurrentTime),
        dayToDisplay
    );
    ...
    if (showTimeMarker) {
        const minutesFromDayStart =
            localCurrentTime.hour * 60 + localCurrentTime.minute;
    ...
    useLayoutEffect(() => {
        ...
        if (view === "day" && scrollContainerRef.current && earliestEventTime) {
            const startMinutes =
                earliestEventTime.hour * 60 + earliestEventTime.minute;
            // Calculate target scroll, ensuring it's not negative
            let targetScrollTop = Math.max(
                0,
                (startMinutes / 30) * SLOT_HEIGHT
                        ...
                        >
                            ...
                            <AriaCalendarGridBody className="[&_tr]:last-of-type]:border-none [&_td]:p-0 [&_tr]:border-b-4 [&_tr]:border-transparent">
                                {(date) => (
                                    <CalendarCell
                                        date={date}
                                        isHighlighted={highlightedDates.has(
                                            date.toString()
                                        )}
                                    />
                                )}
                    ...
                    <div className="flex flex-col gap-5 border-t border-secondary px-6 py-5">
                        ...
                        <section className="flex flex-col gap-2">
                            ...
                            <div className="text-sm text-tertiary">
                                <p>
                                    Sienna is inviting you to a scheduled Zoom
                                    meeting.
                                </p>
                                <br />
                                <p>
                                    Topic: Product demo for the new dashboard
                                    and Q&A session.
                                </p>
                                <br />
...
export const Calendar = ({
    ...
    const hourOnlyFormatter = useDateFormatter({
        hour: "numeric",
        hour12: true,
    });
