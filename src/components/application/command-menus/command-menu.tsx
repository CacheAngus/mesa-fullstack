...
export type CommandMenuGroupType = {
    id: string;
    title?: string;
...
interface CommandMenuContextType
    extends Omit<CommandMenuRootProps, "children"> {
    /**
...
interface CommandMenuRootProps
    extends Omit<
            ListBoxProps<CommandMenuGroupType>,
            "children" | "selectedKeys" | "className"
        >,
...
const CommandMenuRoot = ({
    ...
    const itemsWithShortcutKeys = flatItems.filter(
        (item) => item.shortcutKeys?.length
    );
    const itemsWithShortcutKeysFormatted = itemsWithShortcutKeys.map(
        (item) => ({
            ...item,
            shortcutKeys: item.shortcutKeys!.map((key) => {
                if (key.startsWith("⌘"))
                    return key.replace("⌘", key.length > 1 ? "meta+" : "meta");
    ...
    const useHotkeysRef = useHotkeys(
        itemsWithShortcutKeysFormatted.map((item) =>
        ...
        (keyboardEvent, hotkeysEvent) => {
            ...
            const item = itemsWithShortcutKeysFormatted.find((option) => {
                const parsedHotkey = parseHotkey(
                    option.shortcutKeys?.join("+")
                );
    ...
    useHotkeys(shortcut?.split("").join("+").replace("⌘", "meta") ?? "", () =>
        ...
        >
            ...
            >
                <AriaAutocomplete
                    ...
                    filter={filter === false ? undefined : filter || contains}
                >
                ...
                <AriaModal
                    className={(state) =>
                        cx(
                            ...
                            typeof dialogClassName === "function"
...
interface CommandMenuListProps
    extends Omit<ListBoxProps<CommandMenuGroupType>, "items"> {}
...
export const CommandMenuList = ({
    ...
    const { hasFooter, emptyState, ...listBoxProps } =
...
interface CommandMenuSectionProps
    extends ListBoxSectionProps<CommandDropdownMenuItemProps> {
    title?: string;
}
...
export const CommandMenuSection = ({
    ...
    return (
        <AriaListBoxSection
            {...props}
            className={cx(
                "border-b border-secondary pb-2 last:border-transparent",
                props.className
            )}
        >
            {title && (
...
interface CommandMenuPreviewProps
